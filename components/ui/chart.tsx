...
// FIXED: Better type definitions for payload - using Recharts' actual types
...
// FIXED: Complete props interface with all properties
interface ChartTooltipContentProps extends React.ComponentProps<'div'> {
  ...
  nameKey?: string;
  labelKey?: string;
  labelClassName?: string; // FIXED: Added this missing prop
  // These come from Recharts Tooltip
  active?: boolean;
  ...
  (
    {
      ...
      label: propLabel,
      labelFormatter,
      labelClassName, // FIXED: Now properly destructured
      formatter,
      color,
      nameKey,
      labelKey,
    ...
    // FIXED: Safely check if payload is an array
    const payloadArray = Array.isArray(payload) ? payload : [];

    const tooltipLabel = React.useMemo(() => {
      if (hideLabel || payloadArray.length === 0) {
        return null;
      }
      ...
      const value =
        !labelKey && typeof propLabel === 'string'
          ? config[propLabel as keyof typeof config]?.label || propLabel
          : itemConfig?.label;
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payloadArray.map((item, index) => {
            ...
            const indicatorColor = color || typedItem?.payload?.fill as string || typedItem?.color;

            return (
              <div
                key={typedItem?.dataKey?.toString() || index}
                className={cn(
...
// FIXED: Better type for legend payload
